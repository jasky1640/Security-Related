...
//-------------------------------------------------------------------
SnakeOil::SnakeOil(unsigned int seed) :
...
void SnakeOil::run(int argc, char* argv[]) {
    ...
    key = keyShare[ keyIndex1 ] ^ keyShare[ keyIndex2 ];
    cout << "Master key:\n";
...
//-------------------------------------------------------------------
void SnakeOil::readKeySharesFile(const char* keyFile) {
...
//-------------------------------------------------------------------
void SnakeOil::readPlaintext(const char* inFile) {
    ifstream in(inFile);
    if (!in)
...
void SnakeOil::readIVCiphertext(const char* inFile) {
    ifstream in(inFile);
    if (!in)
        throw CryptoException(
...
//-------------------------------------------------------------------
void SnakeOil::doKeyGen(const char* keyFile) {
